...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'CountryStateCity');
}
...
async function cscGet(apiKey: string, path: string): Promise<unknown> {
  ...
  // The useful part ("Invalid country code") was already in the body, wrapped
  // in a JSON envelope the caller had to parse out of a string (fleet #712).
  if (!res.ok) throw await httpError(res, 'CountryStateCity API');
  return parseJson(res, 'CountryStateCity');
}
